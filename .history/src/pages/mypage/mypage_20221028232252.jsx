...
const MyPage = ({authService}) =>{
  ...
  const [liarScore, setLiarScore] =useState(initLiarScore);
  const handleUpdate = (updated) =>{
    setMyItems(myItems.map((t) => (t.id === updated.id ? updated : t)));
  }
  const handleActive = () => {
    const plusScore= myItems.filter((i) => i.collection === 'liarplus'&& i.status === 'active').length;
    const minusScore = myItems.filter((i) => i.collection === 'liarminus'&& i.status === 'active').length;
    setLiarScore(plusScore-minusScore);
    console.log(`plus ${plusScore},minus ${minusScore}`);
  }
            ...
            <div className={styles.profileStatus}>
                ...
                <div className={styles.status}>
                    <div className={styles.wallet}>
                        <div className={styles.address} onClick={handleActive}>liar Score 새로고침 : {liarScore}</div>
