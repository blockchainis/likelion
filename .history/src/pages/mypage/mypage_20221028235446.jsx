...
const MyPage = ({authService}) =>{
  ...
  const [myStatus, setMystatus] = useState({myItems:initItems, myScore:2})
  // activeitem
  ...
  const handleUpdate = (updated) =>{
    setMystatus({ myItems : myStatus.myItems.map((t) => (t.id === updated.id ? updated : t)), myScore: myStatus.myItems.filter((i) => i.collection === 'liarplus'&& i.status === 'active').length-myStatus.myItems.filter((i) => i.collection === 'liarminus'&& i.status === 'active').length+10} );
  }
