...
const MyPage = ({authService}) =>{
  ...
  const handleUpdate = (updated) =>{
    setMystatus({ myItems : myItems.map((t) => (t.id === updated.id ? updated : t)), myScore: myItems.filter((i) => i.collection === 'liarplus'&& i.status === 'active').length-myItems.filter((i) => i.collection === 'liarminus'&& i.status === 'active').length} );
  }
