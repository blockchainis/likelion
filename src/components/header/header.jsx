  ...
  async function loginWithKaikas() {
    ...
    try {
      const accounts = await toast.promise(
        klaytn.enable(),
        { pending: "kaikas 지갑연동 중" },
        { closeButton: true }
      );
      setUser(accounts[0]);
      localStorage.setItem("_user", accounts[0]);
      toast.success(
        `${accounts[0].slice(0, 5)}...${accounts[0].slice(-5)}님 환영합니다.`
      );
    } catch {
      toast.error("로그인 실패!");
    }
    ...
    <div id="header">
      <header className={styles.header}>
        ...
        <div className={styles.buttons}>
          {user && (
            <Link to="/shop">
              <button className={shop ? styles.active : styles.button}>
                shop
              </button>
            </Link>
          )}
          {user && (
            <Link to="/mypage">
              <button className={mypage ? styles.active : styles.button}>
                my page
              </button>
            </Link>
          )}
