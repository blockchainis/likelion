...
function App() {
  ...
  useEffect(() => {
    ...
    const handleChangeAccounts = () => {
      ...
      if (user !== changedAccount) {
        toast.success(
          `${changedAccount.slice(0, 5)}...${changedAccount.slice(
            -5
          )}님 으로 변경되었습니다.`
        );
  ...
  const initscoreLimit = [
    initItems.filter((i) => i.collection === "liarplus").length,
    initItems.filter((i) => i.collection === "liarplus").length,
  ];
  const [scoreLimit, setScoreLimit] = useState(initscoreLimit);
  const updateScoreLimit = () => {
    setScoreLimit([
      myStatus.myItems.filter((i) => i.collection === "liarplus").length,
      myStatus.myItems.filter((i) => i.collection === "liarminus").length,
    ]);
  };
  ...
  const [myStatus, setMystatus] = useState({
    ...
    myScore:
      initItems
        ...
        .filter((i) => i.collection === "liarplus").length -
      initItems
        .filter((i) => i.status === "active")
        .filter((i) => i.collection === "liarminus").length,
  });
