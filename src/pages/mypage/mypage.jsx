...
const MyPage = ({
  ...
  const handleUpdate = (updated) => {
    setMystatus({
      myItems: myStatus.myItems.map((t) => (t.id === updated.id ? updated : t)),
      myScore:
        myStatus.myItems.filter(
          (i) => i.collection === "liarplus" && i.status === "active"
        ).length -
        myStatus.myItems.filter(
          (i) => i.collection === "liarminus" && i.status === "active"
        ).length,
    });
  };
  const updateScore = () => {
    setMystatus({
      myItems: myStatus.myItems,
      myScore:
        myStatus.myItems.filter(
          (i) => i.collection === "liarplus" && i.status === "active"
        ).length -
    ...
    <section className="all">
      ...
      <section className={styles.contents}>
        <div className={styles.profileStatus}>
          <div className={styles.profile}>
            ...
            <div className={styles.profileName}>
              {nickname} <button onClick={editNickname}>edit</button>
            </div>
          </div>
          ...
          <div className={styles.status}>
            <div className={styles.wallet}>
              <div className={styles.address}>
                liar Score : {myStatus.myScore}
              </div>
              <div className={styles.ownedCoin}>23342coin</div>
            </div>
            <div className={styles.activeItems}>
              {activeItems.length === 0 && (
                <div className={styles.noitem}>착용 아이템 없음</div>
              )}
              {activeItems.map((i) => (
                <ActiveItem key={i.id} item={i} handleUpdate={handleUpdate} />
              ))}
            </div>
        ...
        <div className={styles.allItems}>
          ...
          {filtered.map((i) => (
            <Item key={i.id} item={i} handleUpdate={handleUpdate} />
          ))}
        </div>
      </section>
      <Footer />
